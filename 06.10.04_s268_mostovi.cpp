...
Novi zadatak:
Neprijatelj napada mostove, tj. ceste.
...
int main() {
  ...
  for (int i = 0, j = n; i < m; ++i) {
    ...
    Node intermediate;
    intermediate.neighbors.push_back(a);
    intermediate.neighbors.push_back(b);
    graph.push_back(intermediate);

    graph[a].neighbors.push_back(j);
    graph[b].neighbors.push_back(j);
    ++j;
  }
